...
This provides steps and code for data cleaning, management, and analysis of __Continuity of Essential Health Services__ assessment survey. [See here for the questionnare](https://www.who.int/teams/integrated-health-services/monitoring-health-services/frontline-service-readiness-assessments). <span style="color: red;">
(Standard module version as of 5/12/2021) </span>   
...
__FOUR parts must be updated, based on the minmum country-specific adaptation.__    
...
3. Staff variables in __E.2. Construct analysis variables__ per section 2     
4. Option for sampling weights in __E.3. Merge with sampling weight __ per sentinel facility selection design</span>   
...
<span style="color: red;">See Section __E.2-Addendum__ (added on August 2, 2021)<span> 
...
<span style="color: blue;">Rename indicators ending with sub-question numbers with more friendly names. These names are used in the dash board. __Thus, it is important to ensure the indicator names are correct, if questionnaire is adapted beyond minimum requirements.__ (Addendum on August 2, 2021)<span>  
...
dta<-dta%>%
    rename(
        ...
        xout_decrease_num	 = 	xout_decrease_num	, 
        xout_decrease	 = 	xout_decrease	, 
        ...
        xcvd_pthbsi__followup	 = 	xcvd_pthbsi__003	, 
        ...
        xdrug__paracetamol	 = 	xdrug__004	, 
        xdrug__carbamazapine	 = 	xdrug__005	, 
        xdrug__amoxicillin	 = 	xdrug__006	, 
        ...
        xcovax_train__report_adverse	 = 	 xcovax_train__003	, 
...
```{r calculationtest2, echo=FALSE}
#check sampling weight
table(dta$weight)
summary(dta$weight)

#Question: does the code run with a fake var name? YES
dtasummaryx1<-dtatempx%>%
    mutate(weight=1)%>%
    group_by(country, round, month, year)%>%
    summarize_at(vars(starts_with("nodata")),
...
```{r calculationtest3, echo=FALSE, eval=FALSE}
#check sampling weight
table(dta$weight)
summary(dta$weight)

#Question: does it work with factor variables? NO
dtasummaryx1<-dtatempx%>%
    mutate(weight=1)%>%
    group_by(country, round, month, year)%>%
    #change to factor
    mutate(xcovax=as.factor(xcovax))%>%
    summarize_at(vars(starts_with("xcovax")),
                 funs(mean(., na.rm = TRUE)))%>%
    ungroup()%>%
    mutate(group="All", grouplabel="unweighted")
...
dtasummaryx2<-dtatempx%>%
    ...
    summarize_at(vars(starts_with("xcovax")),
                 funs(weighted.mean(., weight, na.rm = TRUE)))%>%
    ungroup()%>%
    mutate(group="All", grouplabel="weighted")

dtacheck<-rbind(dtasummaryx1, dtasummaryx2)
head(dtacheck)
```

```{r calculationtest, echo=FALSE}
table(dta$weight)
summary(dta$weight)
...
dtasummaryy1<-dtatempy%>%
    mutate(weight=1)%>%
    group_by(country, round, month, year)%>%
    summarize_at(vars(starts_with("y")),
                 funs(sum(., na.rm = TRUE)))%>%
    ungroup()%>%
    mutate(group="All", grouplabel="")

dtasummaryy2<-dtatempy%>%
    group_by(country, round, month, year)%>%
    summarize_at(vars(starts_with("y")),
...
dtacheck<-rbind(dtasummaryy1, dtasummaryy2)
```
