...
local surveyid 			 447349 /*LimeSurvey survey ID*/
...
import delimited "$downloadcsvdir/LimeSurvey_COVID19HospitalReadiness_EXAMPLE_R1.csv", case(preserve) clear /*THIS LINE ONLY FOR PRACTICE*/
	...
	***CHECK: this is an empty row. There should be none	
	...
	foreach var of varlist Q1BSQ001comment Q102 Q103 Q108 Q902 Q903 {
		replace `var'=""
		}		
		replace Q109 =. 	
...
export excel using "$chartbookdirpartner\WHO_COVID19HospitalReadiness_Chartbook.xlsx", sheet("Facility-level raw data") sheetreplace firstrow(variables) nolabel
...
*****E.1. Country speciic code local 
	...
	***** MUST REVIEW CODING FOR THE FOLLOWING FOUR GROUPS OF VARIABLES */
	...
	/*DEFINE LOCAL FOR THE FOLLOWING*/ 
		...
		local maxdrug 			 14 /*total medicines asked in q401*/ 
...
*****E.4. Export clean facility-level data to chart book 
	...
	export excel using "$chartbookdirpartner\WHO_COVID19HospitalReadiness_Chartbook.xlsx", sheet("Facility-level cleaned data") sheetreplace firstrow(variables) nolabel
...
export excel using "$chartbookdirpartner\WHO_COVID19HospitalReadiness_Chartbook.xlsx", sheet("Indicator estimate data") sheetreplace firstrow(variables) nolabel keepcellfmt
