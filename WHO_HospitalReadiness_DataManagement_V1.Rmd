...
__REQUIRED country-specific changes.__     
1. Directories and local macro in __A. SETTING__ per survey implementation information     
2. Local macro in __E.1. Country speciic code local__ per country-specific questionnaire  
...
surveyid<-447349        #LimeSurvey survey ID
...
dta<-dta%>%
	rename_at(.vars = vars(starts_with("q509_")), 
              .funs = funs(sub("q509_", "xipcitem__", .)))%>%	
...
dta<-dta%>%
    ...
    mutate_at(vars(starts_with("xrdt_")), 
              funs(ifelse(xrdt!=1, NA, .)))%>%
    mutate(
        xonsite= xpcr==1 | xrdt==1,
	    xonsite_waste= q611==1,
	    xonsite_ready= (xpcr_equip==1 | xrdt_equip==1) & xonsite_waste==1
    )%>%
    mutate_at(vars(starts_with("xonsite_")), 
              funs(ifelse(xonsite!=1, NA, .)))%>%
    mutate(
    ...
    mutate_at(vars(starts_with("xoffsite_")), 
              funs(ifelse(xoffsite!=1, NA, .)))
...
dta<-dta%>%
	mutate(	
    	...
    	xequip_anymalfunctin__003 = q701_003_001>=1 & (q701_003_001!=q701_003_002),
    	xequip_anymalfunctin__004 = q701_004_001>=1 & (q701_004_001!=q701_004_002)
...
dtasummary<-dtasummary%>%
    ...
    mutate_at(vars(starts_with("xocc"), starts_with("xcovid_occ"), 
                   xpcr_capacity, ends_with("_score")), 
              funs(round((./100), 0)))%>%
    mutate(
