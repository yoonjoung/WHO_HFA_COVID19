...
dta<-dta%>%
    ...
    mutate_if(is.logical, as.numeric)%>%
    mutate_at(vars(starts_with("xabsence_"), starts_with("xhr_"), starts_with("xtraining")), 
              ~replace(., is.na(.)==TRUE, 0))
...
dta<-dta%>%
    mutate(
        opt_increase_num=rowSums(dta[colnames(select(dta, starts_with("opt_increase__")))], na.rm=TRUE ) , 
        opt_decrease_num=rowSums(dta[colnames(select(dta, starts_with("opt_decrease__")))], na.rm=TRUE ) , 
        opt_nochange_num=rowSums(dta[colnames(select(dta, starts_with("opt_nochange__")))], na.rm=TRUE ) , 
        opt_increase = opt_increase_num>=1, 
        opt_decrease = opt_decrease_num>=1    )%>%
    mutate_if(is.logical, as.numeric)%>%
    mutate_at(vars(starts_with("xopt_increase_num"), opt_increase, opt_decrease), 
              ~replace(., is.na(.)==TRUE, 0))%>%
    mutate(
        opt_change=NA, 
            opt_change=ifelse(opt_decrease==0 & opt_increase==0, 1, opt_change), #/*no change in any*/
            opt_change=ifelse(opt_decrease==1 & opt_increase==1, 2, opt_change), #/*mixed*/
            opt_change=ifelse(opt_decrease==1 & opt_increase==0, 3, opt_change), #/*no service volume increased + at least one decreased*/
            opt_change=ifelse(opt_decrease==0 & opt_increase==1, 4, opt_change) #/*no service volume decreased + at least one increased*/
    )

    	###***** REASONS for OPT volume changes

        temp1<-dta%>%select(starts_with("q410_"))%>%
            rename_all(.funs = funs(sub("q410_", "xopt_increase_reason__", .)))%>%
            mutate_all( funs(ifelse(. ==1 , 1, 
                                  ifelse(. !=1, 0, .))) )%>%
            mutate_all(~replace(., is.na(.), 0)) 
        
        temp2<-dta%>%select(starts_with("q411_"))%>%
...
dta<-dta%>%
    mutate(    	
    	...
    	xresto_imp_preg = ifelse(q420_001>=4, NA, xresto_imp_preg), 
    	xresto_imppln_preg = ifelse(q420_001>=4, NA, xresto_imppln_preg), 

    		
    	xresto_imp_immunization = ifelse(q420_002>=4, NA, xresto_imp_immunization), 
    	xresto_imppln_immunization = ifelse(q420_002>=4, NA, xresto_imppln_immunization),
    	
    	xresto_imp_chronic = ifelse(q420_003>=4, NA, xresto_imp_chronic),
    	xresto_imppln_chronic = ifelse(q420_003>=4, NA, xresto_imppln_chronic)
    )%>%
    mutate_if(is.logical, as.numeric)%>%
    mutate_at(vars(starts_with("xrest")), 
              ~replace(., is.na(.)==TRUE, 0))
```
...
dta<-dta%>%
	mutate(	
        xipcpp= q501==1)%>%
    rename_at(.vars = vars(starts_with("q503_")), .funs = funs(sub("q503_", "xsafe__", .)))%>%	
    mutate_at(vars(xipcpp, starts_with("xsafe__")), 
              ~replace(., is.na(.)==TRUE, 0))%>%
    mutate(
        xsafe= q502==1,
            max=9,
    ...
    mutate_if(is.logical, as.numeric)
        ...
        temp2<-dta%>%select(starts_with("q507_"))%>%
            rename_all(.funs = funs(sub("q507_", "xppe_some__", .)))%>%
            mutate_all( funs(ifelse(. ==1 , 1, 
                                  ifelse(. >1, 0, .))) )%>%
            mutate_all(~replace(., is.na(.), 0)) 
