...
*Based on the February 25, 2022 Q version 
*(which has one difference compared to Oct 29, 2021 version)
...
*****B.4. Drop duplicate cases 
	...
	*gen double submitdate = clock(submitdate_string, "YMDhms") /*"clock" line with different mask*/
	...
	list Q105 Q106 submitdate* startdate datestamp if duplicate!=0  
	...
	*****drop duplicates before the latest submission */
	egen double submitdatelatest = max(submitdate) if duplicate!=0  , by(Q101) /*LATEST TIME WITHIN EACH DUPLICATE YC edit 6/29/2021*/					
		...
		*format %tcnn/dd/ccYY_hh:MM:SS submitdatelatest /*"format line with seconds*/
		...
		list Q105 Q106 submitdate* if duplicate!=0
	
	/*YC edit 5/19/2022
		.                 list Q105 Q106 submitdate* if duplicate!=0   

			 +-------------------------------------------------------------------------------+
			 | Q105   Q106        submitdate_string           submitdate    submitdatelatest |
			 |-------------------------------------------------------------------------------|
		...
		* data managers must check if this is a real facility or not - i.e.,
		*		is it actual data entry without facility ID?
		*		is it empty data entry thus there is no facility ID?
